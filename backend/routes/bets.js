...
// GET /api/bets/match/:matchId — palpites de todos (só após o jogo fechado)
router.get('/match/:matchId', auth, async (req, res) => {
  try {
    ...
    if (!betting_closed && !is_finished) {
      return res.status(403).json({ error: 'Os palpites ficam visíveis após o fechamento das apostas.' });
    }
